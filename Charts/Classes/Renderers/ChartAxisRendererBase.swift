...
{
    public weak var axis: ChartAxisBase?
    ...
    public func computeAxisValues(min min: Double, max: Double)
    {
        ...
        else
        {
            ...
            {
                // Ensure stops contains at least numStops elements.
                axis.entries = [Double](count: n, repeatedValue: 0.0)
            }
            else if axis.entries.count > n
            {
                axis.entries.removeRange(n ..< axis.entries.count)
            }
