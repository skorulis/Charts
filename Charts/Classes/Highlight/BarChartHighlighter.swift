...
public class BarChartHighlighter: ChartHighlighter
{
    ...
    public override func getSelectionDetail(xValue xVal: Double, x: CGFloat, y: CGFloat) -> ChartSelectionDetail?
    {
        if let barData = self.chart?.data as? BarChartData
        {
            ...
            for i in 0.stride(to: barData.dataSets.count, by: 1)
            {
                let dataSet = barData.getDataSetByIndex(i) as! IBarChartDataSet

                guard let entry = dataSet.entryForXPos(xVal)
                    else { continue }
                
                let distance = abs(xVal - entry.x)
            ...
            {
                return ChartSelectionDetail(
                    x: x,
                    y: y,
                    xValue: closestEntry.x,
                    yValue: closestEntry.y,
                    dataSetIndex: closestDataSetIndex,
                    dataSet: barData.getDataSetByIndex(closestDataSetIndex))
            }
        }
