...
public class RealmBaseDataSet: ChartBaseDataSet
{
    ...
    public init(results: RLMResults?, yValueField: String, xValueField: String?, label: String?)
    {
        super.init()
    ...
    {
        self.init(results: results, yValueField: yValueField, xValueField: nil, label: label)
    }
    
    public convenience init(results: RLMResults?, yValueField: String, xValueField: String?)
    {
        self.init(results: results, yValueField: yValueField, xValueField: xValueField, label: "DataSet")
    }
    ...
    public init(realm: RLMRealm?, modelName: String, resultsWhere: String, yValueField: String, xValueField: String?, label: String?)
    {
        super.init()
    ...
    {
        self.init(realm: realm, modelName: modelName, resultsWhere: resultsWhere, yValueField: yValueField, xValueField: nil, label: label)
    }
    ...
    internal func buildCache()
    {
        ...
        _cache.removeAll()
        _cache.reserveCapacity(results.count)
        
        var xValue: Double = 0.0
        for e in results
        {
            _cache.append(buildEntryFromResultObject(e, x: xValue))
            xValue += 1.0
        }
    }
